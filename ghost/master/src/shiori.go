...
import (
	...
	"github.com/apxxxxxxe/gohst/internal/readerstream"
	"github.com/apxxxxxxe/gohst/internal/requesthandlers"
	"github.com/apxxxxxxe/gohst/internal/win32file"
)
...
func load(h C.HGLOBAL, length C.long) C.BOOL {
	...
	curDir := C.GoStringN((*C.char)(unsafe.Pointer(h)), (C.int)(length))
...
func request(h C.HGLOBAL, length *C.long) C.HGLOBAL {
	...
	// せっかくサイズ情報があるので C.GoStringN() を使います。
	// ここは C.GoString() でも大丈夫なはず。
	req_str := C.GoStringN((*C.char)(unsafe.Pointer(h)), (C.int)(req_size))
	...
	if err != nil {
		...
		res = dictionary.ResponseBadRequest()
	} else {
		...
		if err != nil {
			...
			res = dictionary.ResponseInternalServerError()
		}
	}
